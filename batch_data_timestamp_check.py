...
def process_one_tick(args):
    scenario_id, tick_idx, image_type, timestamp_key = args
    scenario_name = scenario_list[scenario_id]
    cav_id_list = list(opencood_train_dataset.scenario_database[scenario_id].keys())
    ...
    return f'Scenario idx : {scenario_id} || tick idx : {tick_idx} || timestamp key : {timestamp_key} || Cav List : {cav_list}'
...
for image_type in image_types_list:
    for scenario_id in range(len(scenario_list)):
        ...
        tick_list.remove('ego')
        for tick_idx,timestamp_key in enumerate(tick_list):
            task_list.append((scenario_id, tick_idx, image_type,timestamp_key))
...
with Pool(processes=max(1, cpu_count() - 4)) as pool:
    results = pool.map(process_one_tick, task_list)

for result in results:
    print(result)


### 이걸 출력해서 Scenario idx, tick idx, timestamp key, cav list를 출력해서 label과 model의 input이 제대로 들어가는지 확인해야함
### train_YSmodel.py에서 print(f"Cav_list at scenario : {batch_data['ego']['scenario_id']}, timestamp_key : {batch_data['ego']['timestamp_key']} ||| {batch_data['ego']['cav_list']}")
### 로 확인해봐야함