...
class CamIntermediateFusionDataset(base_camera_dataset.BaseCameraDataset):
    ...
    def __getitem__(self, idx):
        data_sample = self.get_sample_random(idx)       ## data_sample: odict_keys(['659', '641', '650'])
    ...
    def collate_batch(self, batch):
        ...
        for i in range(len(batch)):
            ...
            senario_id_all_batch.append(ego_dict['scenario_id'])
        ...
        agent_true_loc_all_batch = torch.from_numpy(
            np.concatenate(agent_true_loc_all_batch, axis=0)).unsqueeze(1).float()
        single_bev_all_batch = torch.from_numpy(
            np.concatenate(single_bev_all_batch, axis=0)).unsqueeze(1).float()
        timestamp_all_batch = torch.from_numpy(
            np.concatenate(timestamp_all_batch, axis=0)).unsqueeze(1).float()
