...
if __name__ == '__main__':
    ...
    params = load_yaml(os.path.join(current_path,
                                    './opencood/hypes_yaml/opcamera/Test.yaml'))

    opencda_dataset = CamIntermediateFusionDataset(params, train=True, visualize=True)
    opt = vis_parser()

    data_example = opencda_dataset.get_sample(opt.scene, opt.sample)
    # print(data_example['1179'].keys())  ## base_camera_dataset.py의 get_data_sample 함수의 output dictionary값
